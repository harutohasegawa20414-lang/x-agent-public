...
const SENTIMENT_CONFIG = {
  ...
  encrypted: { label: '🔒 暗号化', color: '#94a3b8', bg: 'rgba(100,116,139,0.1)', border: 'rgba(100,116,139,0.25)' },
}
    ...
    <div style={{ padding: '32px 36px' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <select value={filter.status} onChange={e => setFilter(f => ({ ...f, status: e.target.value }))}
          ...
          <option value="encrypted">暗号化</option>
            ...
            <div key={r.id} className="card" style={{
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ margin: '0 0 4px', fontSize: 13, lineHeight: 1.6, whiteSpace: 'pre-wrap',
                    color: r.is_encrypted ? '#4a5568' : '#cbd5e1',
                    fontStyle: r.is_encrypted ? 'italic' : 'normal',
                  }}>
                    {r.is_encrypted ? '🔒 暗号化のため取得不可' : r.reply_text?.slice(0, 200)}
                  </p>
