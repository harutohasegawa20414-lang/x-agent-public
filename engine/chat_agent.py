...
ユーザーとの対話を通じて投稿の方向性・ターゲット・テーマを自由にヒアリングし、
投稿生成に反映するコンテキストを構築するエージェント。
"""
...
ユーザーとの会話を通じて、投稿の方向性を一緒に決めていく役割です。

以下のルールに従ってください：
- ユーザーが伝える内容を自由に受け取ってください（ターゲット層、テーマ、トーン、訴求ポイントなど何でもOK）
- 一次情報（Google Drive / Notion）に縛られる必要はありません。ユーザーの発言内容を最優先で反映してください
- ユーザーの意図を汲み取り、「こういう方向性ですね」と整理・提案してあげてください
- ユーザーが方向性を固めたら、それを投稿生成に使えるコンテキストとしてまとめてください
...
回答の最後に、ユーザーが投稿の方向性・ターゲット・テーマなどを伝えてきた場合は、
これまでの会話で決まった内容を以下の形式でまとめてください：

[EXTRACTED_CONTEXT_START]
（投稿生成に反映すべき方向性・条件をここにまとめる。例：ターゲット層、テーマ、トーン、訴求ポイントなど）
[EXTRACTED_CONTEXT_END]

まだ雑談や質問の段階で方向性が固まっていない場合は、このブロックは不要です。
方向性が少しでも見えてきたら積極的にまとめてください。
"""
        ...
        try:
            ...
            return {
                "ai_message": "申し訳ありません、エラーが発生しました。しばらく待ってから再度お試しください。",
                "extracted_context": None,
            }
