            ...
            <div className="modal-content" style={{ maxWidth: '500px', width: '100%', padding: '1.5rem', borderRadius: '16px', background: 'rgba(30,30,40,0.9)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.1)', maxHeight: 'calc(100vh - 3rem)', overflowY: 'auto' }}>
                ...
                <form onSubmit={handleSubmit} autoComplete="off" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    <div>
                        ...
                        <input type="text" name="id" value={formData.id} onChange={handleChange} required disabled={isEditMode} autoComplete="off" style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', background: isEditMode ? 'rgba(0,0,0,0.5)' : 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', color: isEditMode ? '#94a3b8' : '#fff', cursor: isEditMode ? 'not-allowed' : 'text' }} />
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value={formData.name} onChange={handleChange} required autoComplete="off" style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff' }} />
                    </div>
                    {isEditMode && (
                    ...
                    <div>
                        ...
                        <input type="password" name="x_api_key" value={formData.x_api_key} onChange={handleChange} required={!isEditMode} placeholder={isEditMode ? '変更しない場合は空欄' : ''} autoComplete="new-password" style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff' }} />
                    </div>
                    <div>
                        <label style={{ display: 'block', marginBottom: '0.5rem', color: '#cbd5e1', fontSize: '0.875rem' }}>API Secret</label>
                        <input type="password" name="x_api_secret" value={formData.x_api_secret} onChange={handleChange} required={!isEditMode} placeholder={isEditMode ? '変更しない場合は空欄' : ''} autoComplete="new-password" style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff' }} />
                    </div>
                    <div>
                        <label style={{ display: 'block', marginBottom: '0.5rem', color: '#cbd5e1', fontSize: '0.875rem' }}>Access Token</label>
                        <input type="password" name="x_access_token" value={formData.x_access_token} onChange={handleChange} required={!isEditMode} placeholder={isEditMode ? '変更しない場合は空欄' : ''} autoComplete="new-password" style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff' }} />
                    </div>
                    <div>
                        <label style={{ display: 'block', marginBottom: '0.5rem', color: '#cbd5e1', fontSize: '0.875rem' }}>Access Token Secret</label>
                        <input type="password" name="x_access_token_secret" value={formData.x_access_token_secret} onChange={handleChange} required={!isEditMode} placeholder={isEditMode ? '変更しない場合は空欄' : ''} autoComplete="new-password" style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff' }} />
                    </div>
